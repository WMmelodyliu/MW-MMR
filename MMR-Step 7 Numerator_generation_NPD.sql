...
CREATE OR REPLACE TABLE
  `wmt-mint-mmr-mw-prod.merch_mw_numerator.non_d82_sales_npd` AS
SELECT
  wm_year_nbr,
  wm_month_nbr,
  visit_type,
...
FROM (
  ...
  FROM (
    ...
    FROM (
      ...
      FROM
        `wmt-mint-mmr-mw-prod.MMR_numerator.mmr2_temp_scan_FY26Mar_NPD` mmr_report_temp_scan --update
      WHERE
    ...
    INNER JOIN (
      SELECT
        *
      FROM
        `wmt-mint-mmr-mw-prod.MMR_numerator.lkp_mmr_hier_id_restatement_FY26Feb_NPD`) lookup_mmr --update
    ON
...
WHERE
  ...
  #4b
CREATE OR REPLACE TABLE
  `wmt-mint-mmr-mw-prod.merch_mw_numerator.d82_sales_npd` AS
SELECT
  wm_year_nbr,
  wm_month_nbr,
  visit_type,
...
WHERE
  ...
  #4c
CREATE OR REPLACE TABLE
  `wmt-mint-mmr-mw-prod.merch_mw_numerator.d82_sales_2_npd` AS -- change
SELECT
  wm_year_nbr,
  wm_month_nbr,
  visit_type,
...
FROM (
  SELECT
    ...
    NULL AS date_added
  FROM
    `wmt-mint-mmr-mw-prod.merch_mw_numerator.d82_sales_npd`
  WHERE
    mmr_hier_id!='Parse'
  -- UNION ALL -- change
  -- SELECT
  ...
  --     *
  --   FROM
  --     `wmt-mint-mmr-mw-prod.merch_mw_numerator.d82_sales`
  --   WHERE
  ...
  #4d
CREATE OR REPLACE TABLE
  `wmt-mint-mmr-mw-prod.merch_mw_numerator.ogp_sales_npd` AS
SELECT
  wm_year_nbr,
  wm_month_nbr,
  mmr_hier_id,
...
FROM (
  SELECT
    ...
    department_builder
  FROM
    `wmt-mint-mmr-mw-prod.merch_mw_numerator.non_d82_sales_npd`
  WHERE
    visit_type=86
    OR (visit_type=1
  ...
  FROM
    `wmt-mint-mmr-mw-prod.merch_mw_numerator.d82_sales_2_npd`
  WHERE
    visit_type=86
    OR (visit_type=1
      AND visit_subtype_code IN (190,
...
GROUP BY
  ...
  #4e
CREATE OR REPLACE TABLE
  `wmt-mint-mmr-mw-prod.merch_mw_numerator.offline_sales_npd` AS
SELECT
  wm_year_nbr,
  wm_month_nbr,
  mmr_hier_id,
...
GROUP BY
  ...
  #4f
CREATE OR REPLACE TABLE
  `wmt-mint-mmr-mw-prod.merch_mw_numerator.5p_sales_npd` AS
SELECT
  wm_year_nbr,
...
FROM (
  ...
  FROM (
    ...
    FROM (
      ...
      FROM
        `wmt-mint-mmr-mw-prod.MMR_numerator.mmr2_temp_5p_sales_FY26Mar_NPD` mmr_report_temp_5p_sales --update
      WHERE
        watch = FALSE
        -- AND wm_year_nbr = "2023"
...
GROUP BY
  ...
  #4g
CREATE OR REPLACE TABLE
  `wmt-mint-mmr-mw-prod.merch_mw_numerator.pt_sales_npd` AS
SELECT
  wm_year_nbr,
  wm_month_num_of_yr AS wm_month_nbr,
  mmr_hier_id,
...
FROM (
  SELECT
    ...
    gtin,
    total_5p_sales,
    NPD_mmr_id AS mmr_hier_id --NLSN/NPD --update
  FROM (
    SELECT
      REPLACE(TRIM(prod_type_nm), '"', "") AS prod_type_nm,
      prod_type_nm_excl,
      Watch,
      prod_type_nm_mmr_heir_id as NPD_mmr_id,--NLSN/NPD --update --'prod_type_nm_mmr_heir_id ?'
      WM_Date_Added,
      mmr_rstmt_cycle
    FROM
      `wmt-mint-mmr-mw-prod.MMR_numerator.lkp_producttype_FY26Mar_NPD`) prod_type_lookup --update
  JOIN (
    SELECT
      catlg_item_id,
      div_id,
...
GROUP BY
  ...
  #4h
CREATE OR REPLACE TABLE
  `wmt-mint-mmr-mw-prod.merch_mw_numerator.ogp_5p_sales_npd` AS
SELECT
  wm_year_nbr,
  wm_month_nbr,
  mmr_hier_id,
  ...
  FROM (
    ...
    FROM (
      SELECT
        ...
        NULL AS OGP
      FROM
        `wmt-mint-mmr-mw-prod.merch_mw_numerator.5p_sales_npd`
      UNION ALL
      SELECT
        CAST(wm_year_nbr AS int64) AS wm_year_nbr,
        wm_month_nbr,
        ...
        NULL AS OGP
      FROM
        `wmt-mint-mmr-mw-prod.merch_mw_numerator.pt_sales_npd`
      UNION ALL
      SELECT
        wm_year_nbr,
        wm_month_nbr,
        ...
        OGP
      FROM
        `wmt-mint-mmr-mw-prod.merch_mw_numerator.ogp_sales_npd` )))
GROUP BY
  wm_year_nbr,
  wm_month_nbr,
  mmr_hier_id;
  #4i
CREATE OR REPLACE TABLE
  `wmt-mint-mmr-mw-prod.merch_mw_numerator.offline_sales_with_mmr_npd` AS
SELECT
  wm_year_nbr,
  mmr_dept_id,
...
FROM (
  SELECT
    ...
    wm_month_nbr
  FROM
    `wmt-mint-mmr-mw-prod.merch_mw_numerator.offline_sales_npd`
  GROUP BY
    wm_year_nbr,
...
JOIN (
  SELECT
    *
  FROM
    `wmt-mint-mmr-mw-prod.MMR_numerator.lkp_mmr_hier_id_restatement_FY26Feb_NPD` --update
  CROSS JOIN (
    SELECT
      *
    FROM
...
ON
  ...
  #4j
CREATE OR REPLACE TABLE
  `wmt-mint-mmr-mw-prod.merch_mw_numerator.offline_sales_final_npd` AS
SELECT
  wm_year_nbr,
  wm_month_nbr,
  mmr_dept_id,
...
FROM (
  ...
  FROM (
    ...
    FROM (
      SELECT
        *
      FROM
        `wmt-mint-mmr-mw-prod.merch_mw_numerator.offline_sales_npd`)offline_sales
    RIGHT JOIN ( 
      SELECT
        *
      FROM
        `wmt-mint-mmr-mw-prod.merch_mw_numerator.offline_sales_with_mmr_npd`) offline_sales_with_mmr
    ON
  ...
  FROM (
    SELECT
      *
    FROM
      `wmt-mint-mmr-mw-prod.merch_mw_numerator.offline_sales_npd`)offline_sales_tbl
  JOIN ( --> JOIN CHECK
    SELECT
      *
    FROM
...
ORDER BY
  ...
  #4k
CREATE OR REPLACE TABLE
  `wmt-mint-mmr-mw-prod.merch_mw_numerator.online_sales_with_mmr_npd` AS
SELECT
  mmr_dept_id,
  mmr_category_group_id,
  mmr_category_id,
...
JOIN (
  SELECT
    *
  FROM
    `wmt-mint-mmr-mw-prod.merch_mw_numerator.ogp_5p_sales_npd`)p5_ogp
ON
  lookup_mmr.mmr_hier_id=p5_ogp.mmr_hier_id;
  #4l
CREATE OR REPLACE TABLE
  `wmt-mint-mmr-mw-prod.merch_mw_numerator.online_sales_final_npd` AS
SELECT
  *
FROM (
  SELECT
    ...
    FROM (
      ...
      FROM (
        SELECT
          ...
          wm_month_desc
        FROM
          `wmt-mint-mmr-mw-prod.merch_mw_numerator.online_sales_with_mmr_npd`
        WHERE
          mmr_dept_id NOT IN (
          '3_02',
          '3_03',
...
CREATE OR REPLACE TABLE
  `wmt-mint-mmr-mw-prod.merch_mw_numerator.FY26Mar_mmr_numerator_bq_npd` AS -- update
SELECT
  fiscal_year,
  wm_year_nbr,
  wm_month_nbr,
          ...
          FROM (
            ...
            FROM (
              SELECT
                *
              FROM
                `wmt-mint-mmr-mw-prod.merch_mw_numerator.offline_sales_final_npd`)offline_final_Sales
            FULL JOIN (
              SELECT
                *
              FROM
                `wmt-mint-mmr-mw-prod.merch_mw_numerator.online_sales_final_npd`) online_final_Sales
            ON
...
CREATE OR REPLACE TABLE wmt-mint-mmr-mw-prod.merch_mw_numerator.FY26Mar_mmr_numerator_bq_npd2 AS --update
SELECT *
FROM wmt-mint-mmr-mw-prod.merch_mw_numerator.FY26Mar_mmr_numerator_bq_npd --update
WHERE Business_Unit= 'GENERAL MERCHANDISE'
